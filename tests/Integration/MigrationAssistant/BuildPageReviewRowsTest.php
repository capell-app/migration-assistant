...
it('flags URL collisions against other open publishing-studio and suggests skip', function (): void {
    if (! Schema::hasColumn('page_urls', 'workspace_id')) {
...
it('dashboard-dashboard_reports workspace collision when a URL is owned by any non-live workspace', function (): void {
    if (! Schema::hasColumn('page_urls', 'workspace_id')) {
        $this->markTestSkipped('Requires publishing-studio package (page_urls.workspace_id column)');
    }

    // Core has no concept of "caller workspace" — it flags any workspace_id != 0 as a workspace
    // conflict. Caller-workspace exclusion requires the publishing-studio-package detector.
    $site = Site::factory()->create();
    $uuid = (string) Str::uuid();
